<template>
  <card>
    ...
    >
      <h1 class="text-xl font-bold">Update Task</h1>
...
export default {
  ...
  methods: {
    ...
    submitForm() {
      axios
        .post(`/api/v1/tasks/update/${this.id}`, {
          ...this.formData,
        ...
        .then((response) => {
          ...
          this.formData = {
            task: response.data.res_data.task,
            date_time: response.data.res_data.date_time,
            status: response.data.res_data.status === 1 ? true : false,
          };
        })
        .catch((error) => {
